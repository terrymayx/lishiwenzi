...
test('V1.11.3 puts current task and next-month control in the right rail', () => {
...
test('V1.11.3 keeps next month visible in the right column and reduces the bottom dashboard to three summaries', () => {
...
test('V1.11.3 gives business industry text an explicit readable parchment palette', () => {
  const css = read('dist/v211.css');
...
test('V1.11.3 version is exposed by the page and UI controller', () => {
  const html = read('dist/index.html');
  const js = read('dist/v211-ui.js');
  assert.match(html, /V1\.11\.3/);
  assert.match(html, /v211\.css\?v=1\.11\.3/);
  assert.match(html, /v211-ui\.js\?v=1\.11\.3/);
  assert.match(js, /VERSION = '1\.11\.3'/);
});
